...
local function visual_selection_range()
  ...
  for row = start_row, end_row do
    ...
    if entry then
      ...
      local tmp_file = vim.fn.fnameescape(tmp_dir .. "/" .. entry.parsed_name .. "/")
      local cmd = string.format("cp %s %s", escaped_path, tmp_dir)
      vim.fn.system(cmd)
    end
  end

  local zip_cmd =
      string.format("cd /tmp && zip -r %s.zip %s", vim.fn.shellescape(tmp_dir), vim.fn.shellescape(tmp_dir_name))
  ...
  local copy_cmd =
      string.format([[osascript -e 'set the clipboard to POSIX file "%s"' ]], vim.fn.fnameescape(tmp_dir))

  local copy_result = vim.fn.system(copy_cmd)
  if vim.v.shell_error ~= 0 then
    vim.notify("Failed to copy files to clipboard: " .. copy_result, vim.log.levels.ERROR)
    return
  end
  vim.notify("Files copied to clipboard", vim.log.levels.INFO)
end
...
local function group_entries(start_row, end_row)
  ...
  return tmp_dir
  -- local zip = string.format("%s.zip", tmp_dir)
  -- local zip_cmd = string.format("cd /tmp && zip -r %s %s", vim.fn.shellescape(zip), vim.fn.shellescape(tmp_dir_name))
  -- local result = vim.fn.system(zip_cmd)
  -- if vim.v.shell_error ~= 0 then
  --   vim.notify("Failed to create zip file: " .. result, vim.log.levels.ERROR)
  --   return
